...
#include <random>
#include "math.h"
#define DATA_SIZE 6 // <<-- vectors of size 6
...
int main(int argc, char** argv) {
    ...
    std::for_each(source_in1.begin(), source_in1.end(), [&](float &i) { i = dist(rng); });
    std::for_each(source_in2.begin(), source_in2.end(), [&](float &i) { i = dist(rng); });
    float ans = 0;
    ...
    // for (int i = 0; i < DATA_SIZE; i++) {
        if (source_hw_results[0] != source_sw_results[0]) {
            ...
            std::cout << "i = " << 0 << " CPU result = " << source_sw_results[0]
