                   ...
                   stream<float> &out_result)
{
    ...
    loop2:
        for (int j = 0; j < var_size / 16; j++)
        { // vector_size is ALWAYS a multiple of 16
#pragma HLS pipeline II = 1
            ...
            sum += temp_rand[0] * temp_orig[0] + temp_rand[1] * temp_orig[1] 
            + temp_rand[2] * temp_orig[2] + temp_rand[3] * temp_orig[3] 
            + temp_rand[4] * temp_orig[4] + temp_rand[5] * temp_orig[5] 
            + temp_rand[6] * temp_orig[6] + temp_rand[7] * temp_orig[7] 
            + temp_rand[8] * temp_orig[8] + temp_rand[9] * temp_orig[9] 
            + temp_rand[10] * temp_orig[10] + temp_rand[11] * temp_orig[11] 
            + temp_rand[12] * temp_orig[12] + temp_rand[13] * temp_orig[13] 
            + temp_rand[14] * temp_orig[14] + temp_rand[15] * temp_orig[15];
        }
                     ...
                     stream<ap_uint512_t> &origin_pipe_6)
{
    ...
    int i = 0;
    while (size >= 512)
    { // Size is ALWAYS a multiple of 16 and so a multiple of 512
        for (int j = 0; j < 16; j++)
        {
#pragma HLS unroll
...
#pragma HLS array_partition variable = points_children block factor = 16 dim = 2
    ...
    levels_loop:
        ...
        queue_loop:
            ...
            // Compute distances of children at level l of points in queue
