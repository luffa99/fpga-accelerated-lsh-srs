...
void generateTree(  std::vector<float,aligned_allocator<float>> &points_coords,
    ...
    else {std::cerr<<"Unable to open file querys.txt\n";exit(1);}
...
int main(int argc, char** argv) {
    ...
    float result_py_2 [dimension*n_query] = {};
    float result_py_3 [dimension*n_query] = {};
    float result_py_4 [dimension*n_query] = {};
    ...
    std::vector<float, aligned_allocator<float> > pts(LOWER_SIZE*n_points_real);   // Points of the data structure, to be first projected
    ...
    std::for_each(pts_orig.begin(), pts_orig.end(), [&](float &i) { i = static_cast <float> (rand()) / static_cast <float> (RAND_MAX)/pow(10,floor(log10(vector_size))); });
    ...
    generateTree(points_coords, points_children, result_py, result_py_2, result_py_3, result_py_4, n_points_real, query, maxlevel, 
        minlevel, n_query, want_output, pts, passedTests);
    ...
    for (int i = 0; i < LOWER_SIZE*AMOUNT; i++) {
        ...
        if (abs(query[i] - result_hw_2[i]) >= 2e-6 && abs(query[i] - result_hw_2[i])/query[i] >= 2e-6) {
            nonmproj6 += 1;
        }
    }
    ...
    std::cout << nonmatching_1/6 << std::endl;
    std::cout << "PY TEST " << ((passedTests < AMOUNT) ? "FAILED" : "PASSED") << std::endl;
    std::cout << passedTests <<std::endl;
    ...
    file_append << nonmproj5 << ';' << nonmproj6 << ';'
    << match<<';'<< match_2<<';'<< match_3<<';'<< match_4<<';'<< nonmatching_1/6 <<';'<< nonmatching_4/6 <<';'
    << dts << ';' << now<< ';' 
    << n_query << ';' << n_points_real << ';' << vector_size << ';'<< float_ms.count() << ';' << time_host << ';'<< time_host/float_ms.count() << std::endl;
